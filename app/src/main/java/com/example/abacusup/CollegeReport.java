...
import com.example.abacusup.Custom.GetResponseColl;
...
public class CollegeReport extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mProgressBar = new ProgressDialog(CollegeReport.this);
    ...
    private void SubmitChildInfo() {
        try {
            ...
            ConnectivityManager connMgr = (ConnectivityManager)
                    CollegeReport.this.getSystemService(Context.CONNECTIVITY_SERVICE);
            NetworkInfo networkInfo = null;
            if (connMgr != null) {
                networkInfo = connMgr.getActiveNetworkInfo();
            }
            if (networkInfo != null && networkInfo.isConnected()) {
                String res = new GetResponseColl().execute(hashMap).get();
                String jsonArray = DataProcessor.getData2(res, 0, this);
                try {
                    JSONObject jsonResponse = new JSONObject(jsonArray);
                    JSONArray jsonMainNode = jsonResponse.optJSONArray("CollDetails");
...
//                    DbAccessLayer myDB = new DbAccessLayer(mDBHelper);
                    for(int i = 0; i<jsonMainNode.length();i++) {
                        ...
                        Registrationmodel.setTotal(total);
                    ...
                    scheme.setText("Total no. of Colleges");
                    ...
                    if (tbtdetails != null) {
                        ...
                        if (receiveQuantities.size() > 0) {
                            for (int i = 0; i < receiveQuantities.size(); i++) {
                                ...
                                sc = receiveQuantities.get(i).getTotal();
